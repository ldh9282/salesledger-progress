...
<head>
    ...
    <title>IBTS 컨버전스 용역 매출 현황 [매출실적]: IYF 영업관리시스템</title>
...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">IBTS 컨버전스 <span id="year"></span>년 <span id="month"></span>월 용역 매출 현황 [매출실적]</span>
				</div>
    ...
    <script>
        ...
        $(document).ready(function () {
			...
			if (!params.get('batch_month')) {
	            ...
	            $.ajax({
	                url: "${pageContext.request.contextPath}/salesResult.ajax/company/IBTS/department/CONVERSION/batch_month/" + yyyymm,
	                method: "GET",
				...
				$.ajax({
                    url: "${pageContext.request.contextPath}/salesResult.ajax/company/IBTS/department/CONVERSION/batch_month/" + params.get('batch_month'),
                    method: "GET",
                    success: function (salesResult) {
                        salesResult.forEach(item => {
                            item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesResult.ajax/company/IBTS/department/CONVERSION/batch_month/" + $('#keyword').val(),
                        method: "GET",
                        success: function (salesResult) {
                            salesResult.forEach(item => {
                                item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $('#btnShowRegisterPage').click(function () {
                const popupUrl = '${pageContext.request.contextPath}/ibts/conversion/salesResultRegister';
            ...
            grid.on('dblclick', function(ev) {
                ...
                const popupUrl = '${pageContext.request.contextPath}/ibts/conversion/salesResultDetail?sales_result_id='+ salesResult.sales_result_id;
            ...
            $('a[data-bs-target="#ibts-conversion-nav"]').attr('class', 'nav-link');
       		...
       		$('ul#ibts-conversion-nav li:eq(3) a').attr('class', 'active');
