...
<head>
    ...
    <title>IYS IDC 용역 매출 현황 [매출추정]: IYF 영업관리시스템</title>
...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">IYS IDC <span id="year"></span>년 <span id="month"></span>월 용역 매출 현황 [매출추정]</span>
				</div>
...
<!--         </section> -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/salesEstimation.ajax/company/IYS/department/IDC/batch_month/" + yyyymm,
                method: "GET",
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesEstimation.ajax/company/IYS/department/IDC/batch_month/" + $('#keyword').val(),
                        method: "GET",
                        success: function (salesEstimation) {
                            salesEstimation.forEach(item => {
                                item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $('a[data-bs-target="#iys-idc-nav"]').attr('class', 'nav-link');
       		...
       		$('ul#iys-idc-nav li:eq(2) a').attr('class', 'active');
